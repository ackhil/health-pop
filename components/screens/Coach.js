    ...
    <div>
      ...
      {out[tab] ? (
        ...
        <Tile style={{ border: `2px solid ${C.line}`, textAlign: "center", marginBottom: 12, padding: 24 }}>
          ...
          <Pill style={{ background: C.purple, border: "none" }} onClick={() => ask(tab)} disabled={loading}>{loading ? "Thinking…" : "Generate ✨"}</Pill>
      ...
      <div style={{ display: "flex", gap: 8 }}>
        ...
        <Pill small style={{ borderRadius: 999, width: 50, padding: 0, height: 50, fontSize: 18, background: C.purple, border: "none" }} onClick={() => q.trim() && ask(null, q)} disabled={loading}>↑</Pill>
      </div>
