...
Founder brought a day-of-real-usage backlog across all 5 screens the night before an investor demo, prioritized "solve the feedback bug first" (it was a missing `schema.sql` re-run) and "use higher models for strategy" — an Opus strategic-triage pass classified every backlog item as buildable-now / needs-a-decision / needs-infra before implementation started, in that order:
